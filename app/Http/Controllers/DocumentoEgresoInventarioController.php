...
use App\DocumentoEgresoInventario;
use App\Existencia_Bodega_Lote;
use App\DocumentoEgresoInventarioDetalle;
use App\DocumentoEgresoInventarioTemporal;
use App\DocumentoEgresoInventarioDetalleTemporal;

class DocumentoEgresoInventarioController extends Controller
{
    public function index()
    {
        ...
        if (!isset($_SESSION['id'])) {
            ...
            $documento = DB::select("SELECT idi.numero, idi.created_at, idi.observaciones, SUM(idid.costo_total) as total FROM inv_doc_eai_temp idi JOIN inv_doc_eai_detalle_temp idid ON idi.numero = idid.numero GROUP BY idi.numero, idi.created_at, idi.observaciones");

            $nombre = $_SESSION['nombre'];

            return view('documentos.egreso_inventario', compact('nombre','bodega','documento'));
        }
    ...
    public function getDocumentosInventario(){
        ...
        $documento = DB::select("SELECT idi.numero, idi.created_at, idi.observaciones, SUM(idid.costo_total) as total FROM inv_doc_eai idi JOIN inv_doc_eai_detalle idid ON idi.numero = idid.numero WHERE $query idi.numero IS NOT NULL GROUP BY idi.numero, idi.created_at, idi.observaciones");
    ...
    public function getData(){
        ...
        $detailsDoc = DB::select("SELECT eb.id, p.id_producto, p.descripcion as producto, eb.cantidad, eb.costo_und, ip.precio_venta, ip.iva, eb.lote, eb.fecha_vto, eb.costo_total, tdb.id_bodega FROM inv_doc_eai_detalle_temp eb JOIN productos p ON eb.id_producto = p.id_producto JOIN inventarios_productos ip ON p.id_producto = ip.id_producto JOIN bodegas_documentos tdb ON tdb.id_tipo_doc_bodega = eb.id_tipo_doc_bodega WHERE eb.numero = $numero");
    ...
    public function createDoc()
    {
        ...
        $notemp = DB::select("SELECT CASE WHEN numero IS NULL THEN 1 ELSE MAX(numero) + 1 END as numero FROM inv_doc_eai GROUP BY numero ORDER BY created_at DESC LIMIT 1");
        ...
        $check = DB::select("SELECT numero FROM inv_doc_eai_temp WHERE numero = $num_check");

        if (empty($check)) {

            // dd('insert 1', $num_check);

            $docInv = new DocumentoEgresoInventarioTemporal();
            ...
            $check_detail = DB::select("SELECT numero FROM inv_doc_eai_detalle_temp WHERE numero = $numero");

            if (empty($check_detail)) {

                // dd('update 1', $numero);

                $docInv = DocumentoEgresoInventarioTemporal::find($numero);
                $docInv->prefijo = 'EAI';
                $docInv->id_usuario = $user;
                $docInv->save();
                ...
                $check_temp = DB::select("SELECT MAX(numero) AS numero FROM inv_doc_eai_temp GROUP BY numero ORDER BY created_at DESC LIMIT 1");

                $num_temp_check = $check_temp[0]->numero;

                $check_num = DB::select("SELECT numero FROM inv_doc_eai_detalle_temp WHERE numero = $num_temp_check");

                if (empty($check_num)) {

                    // dd('update 2', $num_temp_check);
                    
                    $docInv = DocumentoEgresoInventarioTemporal::find($num_temp_check);
                    $docInv->prefijo = 'EAI';
                    $docInv->id_usuario = $user;
                    $docInv->save();
                    ...
                    $temp = DB::select("SELECT CASE WHEN numero IS NULL THEN 1 ELSE MAX(numero) + 1 END as numero FROM inv_doc_eai_temp GROUP BY numero ORDER BY created_at DESC LIMIT 1");
    ...
    public function searchDetail()
    {
        ...
        if (empty($lote)) {
            ...
            $bodDet = DB::select("SELECT * FROM inv_doc_eai_detalle_temp WHERE id_producto = '$producto' AND numero = $numero AND lote IS NULL");

        }else{

            $bodDet = DB::select("SELECT * FROM inv_doc_eai_detalle_temp WHERE id_producto = '$producto' AND numero = $numero AND lote = '$lote'");
    ...
    public function updateDetail()
    {
        ...
        if (empty($lote)) {
            ...
            $bodDet = DB::select("SELECT cantidad, costo_und FROM inv_doc_eai_detalle_temp WHERE id = '$registro'");
            $new_cantidad = $cantidad + $bodDet[0]->cantidad;
            $total = $new_cantidad * $bodDet[0]->costo_und;

            $docInvDet = DocumentoEgresoInventarioDetalleTemporal::find($registro);
        ...
        $detalle = DB::select("SELECT eb.id, p.id_producto, p.descripcion as producto, eb.cantidad, eb.costo_und, ip.precio_venta, ip.iva, eb.lote, eb.fecha_vto, eb.costo_total FROM inv_doc_eai_detalle_temp eb JOIN productos p ON eb.id_producto = p.id_producto JOIN inventarios_productos ip ON p.id_producto = ip.id_producto WHERE eb.id = $registro");
    ...
    public function saveDetail()
    {
        ...
        $docInvDet = new DocumentoEgresoInventarioDetalleTemporal();
        ...
        $detalle = DB::select("SELECT eb.id, p.id_producto, p.descripcion as producto, eb.cantidad, eb.costo_und, ip.precio_venta, ip.iva, eb.lote, eb.fecha_vto, eb.costo_total FROM inv_doc_eai_detalle_temp eb JOIN productos p ON eb.id_producto = p.id_producto JOIN inventarios_productos ip ON p.id_producto = ip.id_producto WHERE eb.numero = $numero ORDER BY eb.created_at DESC LIMIT 1");
    ...
    public function deleteDetail()
    {
        ...
        $doc = DB::delete("DELETE FROM inv_doc_eai_detalle_temp WHERE id = $id");
    }
    ...
    public function store(Request $request)
    {
        ...
        $docInvTempo = DocumentoEgresoInventarioTemporal::find($numero);
        ...
        $docInvTemp = DB::select("SELECT * FROM inv_doc_eai_temp WHERE numero = $numero");

        $docInvDetTemp = DB::select("SELECT * FROM inv_doc_eai_detalle_temp WHERE numero = $numero");

        $docInv = new DocumentoEgresoInventario();
        ...
        foreach ($docInvDetTemp as $key => $value) {
            $docInvDet = new DocumentoEgresoInventarioDetalle();
        ...
        $docInvDetPro = DB::select("SELECT id_producto, cantidad FROM inv_doc_eai_detalle_temp WHERE numero = $numero");
        ...
        $docInvDetPros = DB::select("SELECT * FROM inv_doc_eai_detalle_temp WHERE numero = $numero AND lote IS NOT NULL");
        ...
        $InvDetDel = DB::delete("DELETE FROM inv_doc_eai_detalle_temp WHERE numero = $numero");

		$InvDel = DB::delete("DELETE FROM inv_doc_eai_temp WHERE numero = $numero");
  
        return redirect('regdoceai')->with('success','Documento Ingreso Ajuste Inventario Ingresado Satisfactoriamente.');
    }
    ...
    public function show($numero)
    {
        ...
        $docInv = DB::select("SELECT *, b.descripcion as bodega, u.nombre as usuario, i.created_at FROM inv_doc_eai i JOIN bodegas b ON i.id_bodega = b.id_bodega JOIN system_usuarios u ON i.id_usuario = u.usuario_id JOIN tipos_doc_bodega tb ON i.id_tipo_doc_bodega = tb.id_tipo_doc_bodega WHERE i.numero = $numero");

        $docInvDet = DB::select("SELECT * FROM inv_doc_eai_detalle i JOIN inv_doc_eai b ON i.numero = b.numero JOIN productos u ON i.id_producto = u.id_producto JOIN tipos_doc_bodega tb ON i.id_tipo_doc_bodega = tb.id_tipo_doc_bodega WHERE i.numero = $numero");

        $docInvTot = DB::select("SELECT SUM(costo_total) AS total FROM inv_doc_eai_detalle WHERE numero = $numero");
    ...
    public function destroy($numero)
    {
        ...
        $detail = DB::delete("DELETE FROM inv_doc_eai_detalle_temp WHERE numero = $numero");

        $doc = DB::delete("DELETE FROM inv_doc_eai_temp WHERE numero = $numero");

        return redirect('regdoceai');

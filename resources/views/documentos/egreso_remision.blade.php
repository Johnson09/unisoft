...
function validarProducto(){
    ...
    if (bod == '') {
        ...
        $.get('{{ action('DocumentoEgresoRemisionController@getProducto') }}?id='+id+'&bod='+bod, function(data) {
...
function agregarInventario(){
    ...
    $.get('{{ action('DocumentoEgresoRemisionController@searchDetail') }}?id='+nume+'&pro='+prod+'&lote='+lote, function(data) {
        
        if (data == "") {
            $.get('{{ action('DocumentoEgresoRemisionController@saveDetail') }}?id='+nume+'&bod='+bode+'&pro='+prod+'&can='+cant+'&cos='+cost+'&lot='+lote+'&vto='+vto, function(data) {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    return fetch(`{{ action('DocumentoEgresoRemisionController@updateDetail') }}?id=`+registro+'&can='+cant+'&lot='+lote);
                }else {
...
function eliminarInventario(id_registro){
    ...
    $.get('{{ action('DocumentoEgresoRemisionController@deleteDetail') }}?id='+id_registro, function(data) {});
...
function searchDoc(){
    ...
    $.get('{{ action('DocumentoEgresoRemisionController@getDocumentosInventario') }}?doc='+doc+'&bod='+bod+'&ini='+ini+'&fin='+ini, function(data) {
...
function actualizarDoc(numero){
    ...
    $.get('{{ action('DocumentoEgresoRemisionController@getData') }}?id='+numero, function(data) {
...
<div class="container-fluid">
    <h3 style="text-align: center;">EGRESO POR REMISION</h3>
                ...
                <tbody>
                    @foreach($documento as $doc)
                        <tr>
                            ...
                            <td>
                                <a href="{{action('DocumentoEgresoRemisionController@destroy', $doc->numero)}}" class="btn btn-warning">
                                    Eliminar
                                </a>
                            </td>
                        </tr>
...
<div id="generarModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        ...
        <div class="container-fluid text-center">
            <h2 style="color: #2c53c5;"><b>EGRESO POR REMISION</b></h2>
        </div>
        <hr>

...
function actualizar(id){
    ...
    $.get('{{ action('ClaseController@getDetails') }}?id='+id, function(data) {
        $('#cod').empty();
        $('#des').empty();
...
function validacionId(){
    ...
    $.get('{{ action('ClaseController@getDetails') }}?id='+id, function(data) {
        ...
        if (data == "") {
        	...
        	swal({
                title: "Informacion!",
                text: "Codigo de clase ya existe.",
                icon: "error",
                buttons: "Aceptar",
            });
        }
...
<div class="container-fluid">
    <div class="row justify-content-center text-center">
        <div class="container-fluid text-center">
            <h1 style="color: #2c53c5; margin-top: -0.8em;"><b>REGISTRO DE CLASES</b></h1>
        </div>
        <button class="btn btn-info" data-toggle="modal" data-target="#newModal">Añadir Clase</button>
        <hr>
                    ...
                    <tbody style="text-align: center;">
                        @foreach($clase as $cla)
                            <tr>
                                ...
                                <td>
                                    <button onclick="actualizar('{{ $cla->id_clase }}')" class="btn btn-info" data-toggle="modal" data-target="#actModal">Actualizar</button>
                                </td>
                                <!-- <td>
                                  <a href="{{action('ClaseController@destroy', $cla->id_clase)}}" class="btn btn-warning">Eliminar</a>         
                                </td> -->
                            </tr>
                        @endforeach
                    </tbody>
                    ...
                    <div class="row">
                        <div class="col-xs-6 col-sm-6 col-md-6">
                            <div class="form-group">
                                <input type="text" name="id_clase" id="cod" placeholder="Id Clase" readonly="readonly" class="form-control input-lg" tabindex="1" required="required">
                            </div>
                        </div>
                        <div class="col-xs-6 col-sm-6 col-md-6">
                            <div class="form-group">
                                <select class="selectpicker form-control input-lg" id="gru" data-style="btn-info" tabindex="2" name="id_grupo" required="required">
...
<div id="newModal" class="modal fade">
    <div class="modal-dialog">
        ...
        <div class="modal-content" style="background:;">
            <div class="modal-body" style="font-size: 14px">
                <div style="text-align: center;">
                    <form role="form" action="{{ url('regclas') }}" method="post" id="formulario">
                    @csrf
                        <h2>AGREGAR</h2>
                        <h3>CLASE</h3>
                        <hr>
                    ...
                    <div class="row">
                        <div class="col-xs-6 col-sm-6 col-md-6">
                            <div class="form-group">
                                <input type="text" name="id_clase" id="id_clase" placeholder="Id Clase" class="form-control input-lg" tabindex="1" required="required">
                            </div>
                        </div>
                        <div class="col-xs-6 col-sm-6 col-md-6">
                            <div class="form-group">
                                <select class="selectpicker form-control input-lg" data-style="btn-info" tabindex="2" name="id_grupo" required="required">
                                    <option value="">Grupo</option>
                                    @foreach($grupo as $gru)
                                    <option value="{{ $gru->id_grupo }}">{{ $gru->descripcion }}</option>
                                    @endforeach
